---
name: ui-control
description: Control the UI — open panels, toggle tools, set themes, create custom themes, switch modes/models, and open email reply drafts.
version: 1.1.0
category: system-management
tags: [ui, panels, themes, toggles, controls, modes, email-reply]
status: published
confidence: 0.95
source: user
owner: admin
...
Use this skill when the user wants to interact with the Odysseus UI — opening panels, toggling tools on/off, changing themes, creating custom themes, switching chat modes, switching models, or opening email reply drafts. Think of this as the remote control for the app interface.

Key phrases: "open my memories", "show me my skills", "turn off web search", "change the theme to dark", "switch to agent mode", "switch to deepseek", "make a retro wave theme", "open a reply to that email", "open the gallery".

Do **not** use this skill for:
- Listing, reading, or managing content *inside* a panel — use the appropriate `manage_*` tool for that
- Recording user preferences about toggles — toggle state is persistent on its own

## Prerequisites

This skill requires one native tool:
- `ui_control` — Control the UI with actions: `open_panel`, `toggle`, `open_email_reply`, `set_theme`, `create_theme`, `set_mode`, `switch_model`, `get_toggles`
...
| Toggle a tool on/off | `toggle <name> <on\|off>` | `toggle bash on` |
| Switch to a built-in theme | `set_theme <name>` | `set_theme midnight` |
| Create a custom theme | `create_theme <name> <colors>` | `create_theme "Sunset" {bg: "#...", ...}` |
| Open an email reply draft | `open_email_reply <uid> <folder> <mode>` | `open_email_reply 90186 INBOX reply` |
| Switch chat mode | `set_mode <chat\|agent>` | `set_mode agent` |
| Switch active model | `switch_model <model_name>` | `switch_model "deepseek-v4-flash"` |
| View current toggle state | `get_toggles` | `get_toggles` |
...
1. Built-in presets: `dark`, `light`, `midnight`, `paper`, `cyberpunk`, `retrowave`, `forest`, `ocean`, `ume`, `copper`, `terminal`, `organs`, `lavender`, `gpt`, `claude`, `cute`
2. For custom themes, pick distinctive, evocative hex colors that match the requested aesthetic — do NOT use generic defaults
3. The theme auto-applies after creation — no extra step needed
...
3. This opens the email compose window in the UI for the user to type in
4. Do NOT call `reply_to_email` unless the user explicitly gives body text and wants to SEND immediately
...
5. **set_mode vs switch_model:** `set_mode` toggles between chat and agent conversation modes. `switch_model` changes the AI model for the current chat. They are not interchangeable.
...
- `set_mode agent` switches the chat to agent mode
- `switch_model gpt-4o` changes the active model
